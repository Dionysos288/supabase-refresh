 ...
 *          (3s apart) — pauses while the input is focused
 ...
 *          input blurs, and the other side replies in ~700ms
 *          (ambient loop stays 3s — only your send is snappy)
...
const TIMING = {
	ambientInterval: 3000, // ms between flow messages
	replyAfterSend: 700, // after you send, don't wait for ambient — reply feels realtime
...
export function DatabaseChangesCard() {
	...
	useEffect(() => {
		...
		if (!chat) return;
		if (isPausedRef.current && Math.abs(chat.scrollTop) >= 80) return;
		if (scrollAnim.current !== null) cancelAnimationFrame(scrollAnim.current);
	...
	const handleInputKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
		...
		if (e.key === 'Backspace') {
			e.preventDefault();
			if (typedCharIdx.current === 0) return;
			typedCharIdx.current--;
			setDraft((typedMessage ?? '').slice(0, typedCharIdx.current));
			if (typedCharIdx.current === 0) setTypedMessage(null);
			return;
		}
		...
		if (typedCharIdx.current === 0) {
			while (
				CONVERSATION[flowIdx.current % CONVERSATION.length].user !== 'Alice'
			) {
				insertMessage(nextFlowMessage());
			}
		...
		const message =
			typedMessage ?? CONVERSATION[flowIdx.current % CONVERSATION.length].text;
	...
	const handleSend = (e?: React.FormEvent) => {
		...
		// Ambient is 3s; schedule the reply now so send → answer feels instant
		timers.current.push(
			setTimeout(() => {
				if (!isPausedRef.current) insertMessage(nextFlowMessage());
			}, TIMING.replyAfterSend),
		);
	};
	...
	return (
		...
		>
			...
			<div
				ref={tableBoxRef}
				className="relative z-[2] w-full max-w-[320px] overflow-hidden rounded-xl"
			>
				<AnimatePresence initial={false} mode="popLayout">
					{messages.slice(-1).map((msg) => (
						<motion.div
							key={msg.id}
							initial={
								reducedMotion ? { opacity: 0 } : { opacity: 0, y: '105%' }
							}
							...
							<line
								...
								style={{
									animation: reducedMotion
										? 'none'
										: `rlDashFlow 1s linear infinite ${delay}s`,
								}}
							/>
						</svg>
					))}
			...
			>
				...
				<style>{`
            @keyframes rlBubbleIn { from { opacity: 0; transform: translateY(4px) scale(0.98); } }
            .rl-bubble-in { animation: rlBubbleIn 420ms cubic-bezier(0.25, 1, 0.5, 1) both; transform-origin: bottom; }
