...
export const SolutionsDropdown = () => (
  <div className="flex flex-col xl:flex-row">
    <div className="w-[550px] xl:w-[600px] py-8 px-8 bg-background grid gap-3 grid-cols-2 xl:grid-cols-3">
      {SolutionsData['navigation'].slice(0, 3).map((column) => (
    ...
    <div className="bg-surface-75 flex flex-col w-[550px] xl:w-[480px] border-t xl:border-t-0 xl:border-l">
      <div className="flex flex-col gap-6 py-8 px-10">
        <span className="text-foreground-lighter text-xs uppercase tracking-widest font-mono">
          {SolutionsData['navigation'][3].label}
        </span>

        <div className="flex flex-col gap-4">
          {SolutionsData['navigation'][3].links.map((link) => (
...
const LinksGroup = ({ links, label }: { links: NavLink[]; label: string }) => {
  return (
    <div key={label} className="p-0 flex flex-col gap-6">
      ...
      <ul className="flex flex-col gap-4">
        {links.map(({ icon: Icon, ...link }: NavLink) => (
          <li key={link.text}>
            <Link
              href={link.url!}
            ...
            >
              ...
              <ChevronRight
                strokeWidth={2}
                className="w-3 -ml-1 transition-[transform,opacity] will-change-transform -translate-x-1 opacity-0 group-hover:translate-x-0 group-hover:opacity-100"
              />
            </Link>
          </li>
        ))}
...
const MigrationLinkCard = ({ link }: { link: NavLink }) => {
  ...
  return (
    ...
    >
      ...
      >
        <ChevronRight
          strokeWidth={2}
          className="w-3 transition-[transform,opacity] will-change-transform -translate-x-1 opacity-0 group-hover:translate-x-0 group-hover:opacity-100"
        />
      </motion.div>
    </MotionLink>
  )
